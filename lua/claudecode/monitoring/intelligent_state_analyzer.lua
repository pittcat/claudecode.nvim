...
local function analyze_real_state(terminal_content, backend_info)
  ...
  logger.warn(
    ...
    string.format("Unexpected terminal state: %s, reason: %s", terminal_state, terminal_reason)
  )
...
local function perform_intelligent_analysis()
  ...
  if real_state ~= current_state then
    ...
    if real_state == "executing" and current_state ~= "executing" then
      state_manager.set_state(state_manager.States.EXECUTING, "intelligent_analysis", {
    ...
    elseif real_state == "idle" and current_state == "disconnected" then
      ...
      -- 先转为已连接状态（如果需要的话）
      -- 直接设置为 idle 状态
      local success = state_manager.set_state(state_manager.States.IDLE, "intelligent_analysis", {
        analysis_reason = analysis_reason,
        correction_time = now,
        previous_state = current_state,
    ...
    elseif real_state == "idle" and current_state == "executing" then
      -- 从执行中直接转为空闲
      state_manager.set_state(state_manager.States.IDLE, "intelligent_analysis", { analysis_reason = analysis_reason })
    ...
    elseif real_state == "disconnected" and current_state ~= "disconnected" then
      -- 检测到断开连接状态
      state_manager.set_state(state_manager.States.DISCONNECTED, "intelligent_analysis", {
        analysis_reason = analysis_reason,
        detection_time = now,
        previous_state = current_state,
      })
...
function M.start(config)
  ...
  analyzer_state.timer:start(
    1000,
    analyzer_state.check_interval,
...
function M.get_status()
  return {
    ...
    uptime = analyzer_state.enabled and (vim.loop.hrtime() / 1000000 - analyzer_state.last_analysis) or 0,
  }
end
