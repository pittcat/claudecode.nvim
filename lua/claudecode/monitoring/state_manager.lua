...
local function add_history_entry(old_state, new_state, operation_type, operation_details)
  ...
  logger.info("monitoring", string.format("State history: %s -> %s (%.2fms)", old_state, new_state, duration))
end
...
local function update_metrics(operation_type, duration)
  ...
  logger.info(
    "monitoring",
    string.format("Metrics updated: %s took %.2fms, avg: %.2fms", operation_type, duration, metrics.avg_execution_time)
  )
end
...
function M.set_state(new_state, operation_type, operation_details)
  ...
  logger.info(
    "monitoring",
    string.format("set_state called: %s -> %s (operation: %s)", old_state, new_state, operation_type or "nil")
  )

  -- 检查状态转换是否有效
  if not is_valid_transition(old_state, new_state) then
    logger.warn("monitoring", string.format("Invalid state transition: %s -> %s", old_state, new_state))
    return false
  end

  -- 如果状态没有变化，跳过
  if old_state == new_state then
    logger.info("monitoring", string.format("State unchanged: %s", old_state))
    return true
  end
  ...
  logger.info(
    "monitoring",
    string.format("State changed: %s -> %s (operation: %s)", old_state, new_state, operation_type or "nil")
  )
