...
describe("session_manager", function()
  ...
  before_each(function()
    ...
    _G.os = {
      ...
      time = function()
        return 1691000000 -- Mock current time for testing
      end,
    }
  ...
  describe("get_session_list", function()
    it("should return empty list when no Claude config directory found", function()
      mock_vim.loop.fs_stat = function()
        return nil
      end
    ...
    it("should parse and return session list when files exist", function()
      ...
      mock_vim.loop.fs_scandir = function()
        return "handle"
      end
      local call_count = 0
  ...
  describe("format_session_for_display", function()
    it("should format session info correctly", function()
      ...
      assert.is_true(formatted:find("now") ~= nil) -- Should show "now" for recent times
      assert.is_true(formatted:find("main") ~= nil)
      assert.is_true(formatted:find("Test Summary") ~= nil)
      assert.is_true(formatted:find("10") ~= nil)
    end)

    it("should handle long summaries without truncation", function()
      local session = {
        id = "test-id",
      ...
      assert.is_string(formatted)
      -- New implementation doesn't truncate, lets fzf handle it
      assert.is_true(formatted:find("This is a very long summary") ~= nil)
      assert.is_true(formatted:find("main") ~= nil)
      assert.is_true(formatted:find("5") ~= nil) -- message count
    end)
  end)
